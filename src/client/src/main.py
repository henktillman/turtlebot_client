...
irl_name = "intention2"
n_intentions = 4
irl_model = IntentionGAN(irl_name, env_fn, n_intentions, None, None, checkpoint='{}/{}_model'.format(irl_dir, irl_name))
...
def main():
    rospy.init_node('Turtlebot Client', anonymous=False)
    ...
    state = np.zeros(3)
    target = np.zeros(2)
    while np.linalg.norm(box - state[:2]) <= 0.5 or np.linalg.norm(box - target) <= 0.5:
        box = 1.5*np.array([0.2, 0.8])
        box_angle = np.pi/4# * (np.random.rand() - 0.5)
        target = 1.5*np.array([0.8, 0.2])
